...
const argv = require('yargs')
    ...
    .alias('es', 'eslint')
    .boolean(['watch'])
    .command('mode', 'development or production')
...
// Arguments
const ESLINT = Boolean(argv.env.indexOf('eslint') !== -1 || argv.es || argv.eslint)
...
const FILE_FORMAT = productionMode ? '[hash]' : '[name][hash]'
...
module.exports = {
  ...
  module: {
    ...
    rules: [
        /*{
            test: /routes_to_sitemap\.js$/,
            loaders: [{
                loader: 'file-loader',
                options: {
                  name: 'sitemap_source.json',
                },
            }, {
                loader: 'babel-loader',
                options: {
  ...
  plugins: [
    ...
    new webpack.ProgressPlugin(((percentage, msg) => {
        ...
        if (percentage === 1) msg = 'Ready\n'
        const percent = Math.round(percentage * 100)
        if (percent !== 10) { // 10% happens to come up between other messages, skip it
            process.stdout.write(`${percent}%: ${msg}`)
        }
    })),
  ],
  entry: {
    app: [
      path.join(JS_FOLDER, '/src/main.js')
    ],
  }
}
