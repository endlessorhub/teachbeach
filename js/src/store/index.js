...
export default new Vuex.Store({
    ...
    getters: {
        ...
        showClassCompanyName(state) {
            return !state.logoDimensions || !state.logoDimensions.height || state.logoDimensions.width <= state.logoDimensions.height*4
        },
        isMember: (state) => (membershipId) => {
            return state.user && state.user.memberships && state.user.memberships.some((m) => m.membership === membershipId)
        },
    ...
    mutations: {
        ...
        setGlobals: function (state, globals) {
            state.globals = Object.entries(globals).reduce((acc, [k, v]) => ({...acc, ...{[k]: v === 'True' ? true : v === 'False' ? false : v} }), {})
        },
    ...
    actions: {
        ...
        async syncLogoCompanyProfile(store, id) {
            ...
            if (!store.state.logoCompanyProfile || (store.state.logoCompanyProfile.id !== id && store.state.logoCompanyProfile.slug !== id)) {
        ...
        setInitialdata(store, res) {
            ...
            if(res.data.learner_needs) {
                store.commit('mergeLearnerData', {learnerNeeds: res.data.learner_needs})
            }
            if(res.data.user) {
            ...
            if(res.data.draft_classes && res.data.draft_classes.length) {
                ...
                store.commit('setTeacherLessonType', res.data.last_class.is_private ? 'private' : 'group')
        ...
        loadTeacherMembership(store) {
            ...
            return axios.get('/api/teacher_membership/').then((res) => {
                if(res.data.success) {
        ...
        saveTeacherMembership(store, payload) {
            store.commit('setIsLoadingTeacherMembership', true)
            return axios.post('/api/teacher_membership/', payload).then((res) => {
                if(res.data.success) {
                    store.commit('setTeacherMembership', res.data.membership)
