...
<v-app>
  <div id="app">
      <Snack v-if="globalError" v-model="globalError" />
      <v-layout align-top justify-center row wrap class="inner-app">
        <v-flex xs12 md10 class="text-xs-center shadowed">
          <v-toolbar
              class="app-toolbar"
          ...
          >
            <v-toolbar-title>
              ...
              <a v-else-if="actualLogo.homeUrl" :href="actualLogo.homeUrl">
                <img ref="logoImg" v-if="actualLogo.logoSrc" :alt="actualLogo.logoAlt" :src="actualLogo.logoSrc" style="height:72px;max-width: 100%;" @load="onLoadLogoImg"/>
              </a>
              <router-link v-else-if="actualLogo.logoLink" :to="actualLogo.logoLink" style="text-decoration: none; color: black;">
                <img ref="logoImg" v-if="actualLogo.logoSrc" :alt="actualLogo.logoAlt" :src="actualLogo.logoSrc" style="height:72px;max-width: 100%;" @load="onLoadLogoImg"/>
              </router-link>
            </v-toolbar-title>
            <div class="class-company-name" v-if="actualLogo.classCompanyName && showClassCompanyName">{{actualLogo.classCompanyName}}</div>
            <v-spacer></v-spacer>
            <company-nav-bar
                v-if="isCompanyProfilePage"
                :companySlug="companySlug"
                :onTeacherDashboardClick="onTeacherDashboardClick"
                :onAccountClick="onAccountClick"
            ></company-nav-bar>
            <div v-else class="nav-bar-container">
                <v-toolbar-items class="hidden-md-and-down">
                <v-btn key="home" v-if="actualLogo.homeUrl" flat @click="openExternalLink(actualLogo.homeUrl)">Home</v-btn>
                <v-btn key="teach" v-if="isLoggedIn && (!hideTeachAndLearn || isTeacher)" :input-value="topTeachBtnActive" flat @click="onTeacherDashboardClick">Instruct</v-btn>
                <v-btn key="learn" v-if="isLoggedIn && (!hideTeachAndLearn || isLearner) && !isTeacherOfCompany" flat to="/dashboard/learn">Learn</v-btn>
                <v-btn
                    key="register"
                    flat
                    data-desktop-nav-addclass
                    @click="onClickPlusClass"
                >{{ isLoggedIn ? '+Class' : 'Become an instructor' }}</v-btn>
                <!--v-btn flat to="/venues">+Venue</v-btn-->

                <!--v-btn v-if="!isLoggedIn" flat to="/teachers/join" data-desktop-nav-join>join</v-btn-->
                <v-btn key="login" v-if="!isLoggedIn" flat @click="() => onAccountClick({section: 'login'})" data-desktop-nav-login>login</v-btn>
                <v-btn key="logout" v-if="isLoggedIn" flat @click="onAccountClick" data-desktop-nav-logout>logout</v-btn>
                <!--v-btn flat icon @click="onAccountClick"><v-icon class="material-icons">account_circle</v-icon></v-btn-->
                </v-toolbar-items>
          ...
          <div v-if="actualLogo.classCompanyName" class="powered-by-container"><router-link to="/">powered by CollabSocial</router-link></div>
            <router-view
                v-if="isAuthChecked"
                :city="city"
...
.v-autocomplete__content .v-list .v-subheader {
    font-size: 1.5em;
}
.v-menu__activator {
    div[role=listitem] {
