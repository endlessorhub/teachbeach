<template>
    ...
    >
        ...
        <v-btn @click="() => {saveTeacherLocal(); submit();}" :loading="isLoading">Back to Profile</v-btn>
    </MultipleProfiles>
</template>
...
import MultipleProfiles from '@/components/Teacher/MultipleProfiles'
        ...
        submit () {
            this.saveTeacher().then((res) => {
                if(res) {
                    ...
                    this.$router.push('/dashboard/teach/profile')
                }
            }).catch((e) => {
                console.log('err!', e)
            })
