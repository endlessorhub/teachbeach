<template>
    ...
    <div v-if="message" class="ChatGroup-RecieverContent">
        ...
        <div class="RecieverContent-image">
                    ...
                    <div class="RecieverContent-about">
                        ...
                        <div class="RecieverContent-about-ShareLinks">

                            <span :class="{ 'like-button': message.is_liked }" @click="sendLike()">
                                    <svg v-if="!message.is_liked" width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path
                                ...
                                {{ message.is_liked ? "Liked" : "Like" }}
                            </span>

                            <span v-if="isReplyIcon" @click="sendReplyId()">
                            ...
                            <span v-if="isReplyIcon">
                                <input type="file" :id="message.id" accept="image/png, image/gif, image/jpeg"  class="hidden"  @change="fileInput($event)" />
    ...
    <v-card>
        ...
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="red" @click="removeSelectedComment(message.id)">
                Delete
            </v-btn>
    ...
    methods: {
        ...
        sendReplyId() {
            ...
            if (this.showPostMessage)
                ...
                this.$emit('replyId', this.message.id)
            else
      ...
      reader.readAsDataURL(uploadedImage);
        ...
        async blockSelectedUser(userId) { 
            ...
            const status = await this.blockUser(payload)
        ...
        sendLike() { 
            let payload = {
                "comment_id": this.message.id,
                "type": "COMMENT-LIKE",
                "is_liked": !this.message.is_liked             
            }
